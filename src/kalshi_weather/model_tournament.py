...
function cell(row,key){{if(key==='_close_status_reason')return fallbackCloseReason(row);if(key==='status')return statusBadge(row[key]);if(String(key).endsWith('_dollars'))return money(row[key]);return isTimeKey(key)?formatPT(row[key]):fmt(row[key]);}}
...
function cellClass(key){{const classes=[];if(key==='_close_status_reason')classes.push('reason-cell');if(key==='status')classes.push('status-cell');return classes.join(' ');}}
