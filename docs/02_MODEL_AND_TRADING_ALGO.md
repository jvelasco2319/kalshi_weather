...
`model-health` does not tune the model. It reads stored evidence and reports:

- data readiness and unique joined market dates
- residual mean, MAE, RMSE, standard deviation, and suggested sigma
- calibration Brier score, log loss, buckets, and empirical YES rate
- model-vs-market Brier/log-loss deltas
- fake-money paper fills and replay status

For model-vs-market, the market benchmark uses the YES bid/ask midpoint when
available. If the midpoint is missing, the command uses a clearly labeled
conservative proxy such as YES ask or `1 - NO ask`.

Positive `model_minus_market_brier` means the model outperformed the market
benchmark on that scored sample. Do not interpret this until joined rows and
unique market dates are large enough.

## Separate Model Estimate Comparison

The comparison commands do not create a new production model. They produce one
high-temperature estimate per model source:

```text
settlement_high_estimate_f = max(observed_high_so_far_f, future_high_f)
```

For probabilities, each estimate is passed through the same normal-residual
sampling and bracket-probability logic used by the current model:

```text
same probability model, different weather-model input
```

This keeps the comparison fair and prevents accidental blending. Direct
NOAA/Herbie estimates are optional diagnostics. They are not used for paper
entries, opportunities, or replay unless a future phase explicitly changes that.
